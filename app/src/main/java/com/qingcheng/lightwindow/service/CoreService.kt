...
import com.qingcheng.lightwindow.receiver.ScreenStateReceiver
import com.qingcheng.lightwindow.receiver.SensorListener
import com.qingcheng.lightwindow.util.NotificationBuilder
import com.qingcheng.lightwindow.util.VibratorUtil
...
class CoreService : Service() {
                    ...
                    withContext(Dispatchers.IO) {
                        dataBase.eventDao().getEvents().first()
                            .let {
                                ...
                                NotificationManagerCompat.from(this@CoreService).notify(
                                    mainNoticeId,
    ...
    /**
     * 收集事件数据库流并更新通知和设置闹钟
     * @see AlarmManagerUtil.repeatDay
     * @see updateAlarm
     * @see updateNoticeBuilder
     * */
    private fun setAlarmAndMainNotice() {
        //每天0点更新一下
        AlarmManagerUtil.repeatDay(this, Calendar.getInstance().let { calendar ->
            calendar.time = Date()
        ...
        MainScope().launch {
            withContext(Dispatchers.IO) {
                dataBase.eventDao().getEvents().collect { events ->
                    ...
                    updateAlarm(nextNoticeEvent)
                    //通知有变化才更新
                    if (newNotice != lastNotice) NotificationManagerCompat.from(this@CoreService)
                        .notify(mainNoticeId, mainNotificationBuilder.build())
                }
            }
        }
    ...
    private fun startListener() {
            ...
            onTrigger = {
                it?.let {
                    if (!km.isKeyguardLocked) {
                        if (it.values[2] < -8 && f) {
                            VibratorUtil.vibrate(this@CoreService, 100)
                            startService(
                                Intent(
                                    this@CoreService,
        ...
        screenStateReceiver = ScreenStateReceiver().apply {
            ...
            onScreenOff = {
                sensorListener.disable()
                stopService(Intent(this@CoreService, CalendarWindowService::class.java))
                stopService(Intent(this@CoreService, MainWindowService::class.java))
            }
            enable(this@CoreService)
        }
    }
}
